...
public class SyntaxHelper {
     ...
     */
    private static void checkArray(final File file, final JsonArray array) {
     ...
     */
    private static void checkString(final File file, final String string) {
        if (string.endsWith(",")) {
            throw jsonFormatEx(COMMA_MESSAGE, file.getName());
        } else if (string.endsWith(";")) {
            throw jsonFormatEx(SEMICOLON_MESSAGE, file.getName());
        } else if (COMMENT_PATTERN.matcher(string).find()) {
            throw jsonFormatEx(COMMENT_MESSAGE, file.getName());
        }
    }
}
