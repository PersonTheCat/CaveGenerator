...
import static personthecat.cavegenerator.presets.data.CaveBlockSettings.Fields.integrity;
import static personthecat.cavegenerator.presets.data.CaveBlockSettings.Fields.noise;
import static personthecat.cavegenerator.presets.data.CaveBlockSettings.Fields.states;

public class CaveBlockValidator {

    private CaveBlockValidator() {}

    public static void apply(final ValidationContext ctx, final CaveBlockSettings s, final JsonPath.Stub path) {
        if (s.integrity != null) {
    ...
    public static void between(final ValidationContext ctx, final List<CaveBlockSettings> a, final JsonPath.Stub path) {
        int lastMinHeight = -1;
        int lastMaxHeight = -1;
        boolean foundGuaranteed = false;

        for (int i = 0; i < a.size(); i++) {
            final CaveBlockSettings s = a.get(i);
            if (foundGuaranteed) {
